...
async function main() {
  ...
  const duplicate = await insertEmail({
    message_id: messageId,
  ...
  const classified = await updateEmailClassification(first.row.id, {
    category: "sales_lead",
    confidence: 0.91,
    lead_score: 78,
  ...
  const withDraft = await updateEmailDraft(
    first.row.id,
  ...
  const pending = await updateEmailStatus(first.row.id, "pending_approval");
  assert(pending.status === "pending_approval", "status update should persist");

  const byId = await getEmailById(first.row.id);
